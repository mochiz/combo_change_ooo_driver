...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kcombo_change_ooo_driverUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kcombo_change_ooo_driverUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
