...
NSString * const kcombo_change_ooo_driverJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_combo_change_ooo_driverInjection
{
	...
	[html appendString:kcombo_change_ooo_driverJavascript];
...
{
	// attempt to make well-formed HTML and inject in our combo_change_ooo_driver bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _combo_change_ooo_driverInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to combo_change_ooo_driver support.",url);
				}
			}
