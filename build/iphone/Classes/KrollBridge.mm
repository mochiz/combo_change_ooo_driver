...
@implementation combo_change_ooo_driverObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_combo_change_ooo_driver);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any combo_change_ooo_driver patches in JS before a context is loaded... nice for 
...
{
	// create combo_change_ooo_driver global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_combo_change_ooo_driver = [[combo_change_ooo_driverObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_combo_change_ooo_driver];
	
	NSString *_combo_change_ooo_driverNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_combo_change_ooo_driverNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_combo_change_ooo_driver valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.combo_change_ooo_driver.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
